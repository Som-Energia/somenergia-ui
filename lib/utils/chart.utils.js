...
export const formatDecimal = (
  ...
  formatNumber(
    (Math.round((item + Number.EPSILON) * base) / base)
...
export const formatTooltipLabel = (
  ...
  const hour = (values, addition = 0) =>
    dayjs(values).add(addition, "hour").format("HH")
  ...
  const formatWithHour =
    formatedDay + " " + initialHour + "h - " + finalHour + "h"
  ...
  switch (period) {
    ...
    case "WEEKLY":
    case "MONTHLY":
      return type === "barChart" ? formatedDay : formatWithHour
    default:
      return formatedDay
  }
}
