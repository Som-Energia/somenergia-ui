...
export function buildTicks(minYAxisValue, maxYAxisValue, tickCountValue) {
  return minYAxisValue !== 'auto' && maxYAxisValue !== 'auto'
    ? getNiceTickValues([minYAxisValue, maxYAxisValue], tickCountValue, true)
    : false
}
...
export const formatTooltipLabel = (period, values, type = 'barChart', displaced = false, step_mm = 60) => {
  ...
  const hour = (values, addition=0) => dayjs(values).add(addition, 'millisecond').format('hh:mm')
  let initialHour, finalHour

  const step_sm = step_mm * 60 * 1000
  const formatedDay = formatDay(values)
  if (displaced) {
    initialHour = hour(values, -step_sm)
    finalHour = hour(values)
  } else {
    initialHour = hour(values)
    finalHour = hour(values, step_sm)
  }
